<template>
  <div v-for="task in jsonData.tasks" :key="task.taskID">
    ...
    <a-table
      ...
      :loading="isLoading"
      size="middle"
      :getPopupContainer="(triggerNode) => triggerNode.closest('.custom-table')"
...
const fetchData = async (page: number) => {
  ...
  // need to clear for every api call , bcz globally declared.
...
const exportDataByType = async (dataType: any) => {
  ...
  if (selectedFilter.value.length > 0 && searchKey !== "status") {
    params["status"] = selectedFilter.value[0];
  }

  console.log("running 2 ::: export params :::", params, searchTerm, searchKey);
...
const handleStartSample = (record: any) => {
  ...
  if (record?.id) {
    router.push(`/apc/lims/${record?.id}`);
  } else {
...
const handleSyncSample = (record: any) => {
  // console.log(" handleSelectedTableRowClick", record);
  if (record) {
    router.push(`/apc/lims-editor/${record?.id}`);
  } else {
    alert("Sample data not found. Please try another sample");
  }
};
...
const openPop = async (record: any) => {
  ...
  try {
    ...
    if (taskResultData) {
      ...
      // Initialize modalData as an empty object if it's not already defined
      modalData = modalData || {};

      // Assign fetchedData to modalData
      modalData = { ...fetchedData };

      if (record.status !== "New") {
        // Ensure modalData.meta exists before assigning values to it
        modalData.meta = modalData.meta || {};
...
onUnmounted(() => {
  ...
  subscribedDeviceIds.value.forEach((deviceId) => {
    socketService.leaveRooms([`instrument/apc/${deviceId}/heartbeats/basic`]);
  });
