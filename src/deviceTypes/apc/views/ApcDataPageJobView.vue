              ...
              <a-space>
                ...
                <a-button
                  v-if="
                    record.isSampling &&
                    (record.actionStatus.toLowerCase() === 'in-progress' ||
                      record.actionStatus === 'in-progress') &&
                    record.sampleType === 'Continuous-sample'
                  "
                  @click="openForceDropModal(record, 'Sample Stop', $event)"
                  type="primary"
                  danger
                  size="small"
                >
                  {{ translateRecordsAsPerLanguage("Stop") }}
                </a-button>
                <div>
                  <!-- Abort Button -->
                  <a-button
                      ...
                      <template #overlay>
                        <a-menu style="text-align: start">
                          <a-menu-item
                            ...
                            :key="index"
                            @click="handleActionClick(action, record, $event)"
                            style="width: 180px"
                          >
                            {{ action }}
                          </a-menu-item>
...
import dataJsonData from "../config/apcDataPageJob.json"; // Import the JSON data directly
...
export default defineComponent({
  ...
  setup() {
    ...
    // Debouncing for fetchData to prevent multiple simultaneous calls
    let debounceTimer: ReturnType<typeof setTimeout> | null = null;
    const debouncedFetchData = (page: number) => {
      if (debounceTimer) {
    ...
    const columns = computed(() => [
      ...
      // { title: localizedTitles.value.user, dataIndex: "user" },
      {
        ...
        dataIndex: "meta.sample_type_label",
      },
    ...
    const activeWatchers = ref<Array<() => void>>([]);
    ...
    const getAbortActions = (record: any): string[] => {
      console.log(":::: sampleRecord in Action", record);
      const sampleType = record?.sampleType || "";
      if (
        sampleType === "Sample-Single" ||
        sampleType === "Sample-Continuous Run"
      ) {
        return ["Abort Sample", "Abort Continuous Sample"];
      } else if (
        sampleType === "Flush-Single" ||
        sampleType === "Flush-Continuous Run"
      ) {
        return ["Abort Flush", "Continuous Flush"];
      } else if (sampleType === "Sample-Interval") {
        return ["Abort Current Sample", "Abort Sample Interval"];
      } else {
        return ["Abort"];
      }
    };

    const handleActionClick = (action: string, record: any, event: any) => {
      console.log(":::: action from the action click btn", action);
      // if (action.toLowerCase().includes("abort")) {
      openAbortModal(record, "Abort", event, action);
      // } else {
      //   openForceDropModal(record, "Force Drop");
      // }
    };
    ...
    const openPop = async (record: any) => {
      ...
      if (!record.id) {
        console.error("No task result id found in record.");
        return;
      }
    ...
    const openDeviceActionData = async () => {
      const taskResultId = route.query.task_result_id as string;
      if (taskResultId) {
        const record = {
          id: taskResultId,
        };
    ...
    socketService.handleMessage((incomingHeartbeat: any) => {
      tableData.value.forEach((taskResult: any, index: number) => {
        ...
        // Don't update tasks that are in final status
        ...
        if (
          ...
          !FINAL_STATUSES.includes(taskResult.status)
        ) {
          try {
            Object.assign(tableData.value[index], {
              ...
              activeJobId: processedHeartbeat?.basic?.active_job_id,
              isRecurringSample: processedHeartbeat?.basic?.is_recurring_sample,
            });
      ...
      try {
        ...
        tableData.value = [
          ...response.data.map((taskResult: any) => {
            ...
            taskResult.activeJobId = null;
            taskResult.isRecurringSample = false;
    ...
    const setupWatchers = () => {
      // Clean up all existing watchers first
      activeWatchers.value.forEach((unwatch) => unwatch());
      activeWatchers.value = [];

      // Now create new watchers
      for (let i = 0; i < tableData.value.length; i++) {
        ...
        const unwatch = watch(
          ...
          (
            ...
            if (
              ...
              debouncedFetchData(currentPage.value);
              return;
            }
          }
        );
        ...
        activeWatchers.value.push(unwatch);
      }
    };
