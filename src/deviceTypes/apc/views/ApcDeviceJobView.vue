  ...
  <div class="instrumentPage">
    ...
    <div class="custom-table-scroll">
      <a-table
        ...
        rowKey="id"
        :getPopupContainer="
          (triggerNode: any) => triggerNode.closest('.ant-table-container')
        "
      ...
      >
        ...
        <template #bodyCell="{ column, record }">
          ...
          <template v-if="column.dataIndex === 'edit'">
            <a-tooltip>
              ...
              <EditOutlined
                ...
                :style="{
                  cursor:
                    (record.isSampling && record.activeJobId !== null) ||
                    record.connection_status === 'disconnected' ||
                    (record.connection_status === 'connected' &&
                      !record.is_active)
                      ? 'not-allowed'
          ...
          <template v-if="column.dataIndex === 'reset'">
            <a-tooltip>
              <template #title>
                {{
                  (record.connection_status === "connected" ||
                  ...
                  record.activeJobId !== null
                    ? t("devicePage.Reset", "Reset")
                    : t("devicePage.Cannot Reset", "Cannot Reset")
                }}
              </template>
              <ReloadOutlined
                @click="
...
export default defineComponent({
  ...
  setup() {
    ...
    const fetchPicture = async (id: string | null) => {
      if (!id) {
        // id is null, empty, or undefined — don’t call API
        assetImage.value = "";
        return;
      }
      ...
      try {
        if (taskData.type === "renderTable" && taskData.table?.api) {
          ...
          if (exportType !== undefined) {
            ...
            console.log(":::::search header data", searchHeaders);
    ...
    const handleEditRecord = (record: any) => {
      ...
      router.push(`/apc/instrument/device/${record.id}`);
    };

    const handleResetRecord = (record: any) => {
      const assetId = record.asset_id;
      Modal.confirm({
        title: t("devicePage.Reset", "Reset"),
        icon: h(ReloadOutlined),
        content: `${t(
          "devicePage.Are you sure you want to reset instrument with Asset ID",
          "Are you sure you want to reset instrument with Asset ID"
        )} ${assetId}?`,
        okText: t("commonData.Confirm", "Confirm"),
        cancelText: t("commonData.Cancel", "Cancel"),
        async onOk() {
          console.log("Reset instrument with asset ID:", assetId);
          const resetResponse = await apcService.resetDevice(assetId);
    ...
    socketService.handleMessage((data: any) => {
      tableData.value.forEach((device: any, index: number) => {
        const deviceHeartbeat = processHeartbeat(data.msg);
        ...
        if (deviceID && device.id && deviceID == device.asset_id) {
          try {
            Object.assign(tableData.value[index], {
              ...
              activeJobId: deviceHeartbeat?.basic?.active_job_id,
            });
          } catch (e) {
            console.error(
