  ...
  setup() {
    ...
    const dataPageHeaders = () => {
      ...
      devicePageHeader.push(
        ...
        { field: "count_limits", header: "Counts Limits" },
        {
          field: "flow_limits",
    ...
    socketService.handleMessage((incomingHeartbeat: any) => {
      tableData.value.forEach((taskResult: any, index: number) => {
        ...
        if (
          ...
          try {
            if (tableData.value[index]?.actionStatus === "Aborted") {
              tableData.value[index].status =
                tableData.value[index]?.actionStatus;
    ...
    const fetchData = async (page: number) => {
      ...
      fetchSearchKeys();
      console.log("required", searchKey);
      console.log("required", searchTerm);
      console.log("required", selectedStatuses.value);
      ...
      if (selectedStatuses.value.length > 0) {
        ...
        searchHeaders.value.push({
          searchKey: selectedStatuses.value[0],
        });
      }
      console.log("required searchHeaders", searchHeaders.value);
      console.log("params at data page ", params, searchKey, searchTerm);
    ...
    const openAbortModal = (record: any, actionType: any, event: any) => {
      ...
      //console.log("openAbortModal:::", connectionData.value);
    };
...
/* Optional: Ensure the table body is scrollable vertically without affecting horizontal scroll */
.data-table .ant-table-body {
  /* max-height: 500px; Adjust based on your desired height */
  z-index: -9999; /* Ensure it appears above other elements */
  overflow-y: auto;
}

/* Optional: Handle horizontal scrolling */
.data-table .ant-table {
  overflow-x: auto;
}

.ant-table-column-title .ant-table-filter-trigger {
  display: flex;
  align-items: center;
...
.ant-table-column-title .ant-table-filter-trigger:hover {
  color: #4caf50; /* Change color on hover */
}

/* Adjust table filter icon appearance */
.data-table-column-title .ant-table-filter-trigger .anticon {
  font-size: 28px;
  z-index: 9999; /* Ensure it stays above other UI elements */
  overflow-y: visible;
}
