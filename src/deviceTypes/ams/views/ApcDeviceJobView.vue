...
export default defineComponent({
  ...
  setup() {
    ...
    const rowClassName = (record: any) => {
      // console.log(
      //   "selected row class - selected-row",
      //   selectedDevice.value,
      //   record.id
      // );
      return selectedDevice.value == record.id ? "selected-row" : "";
    };
    ...
    const fetchPicture = async (id: string) => {
      try {
        ...
        if (response) {
          ...
          const buffer = data.medium_definition_image.data;
    ...
    const handleSwitchToggle = (record: any, event: any) => {
      ...
      Modal.confirm({
        title: `${t(
          "devicePage.Are you sure you want to",
          "Are you sure you want to"
        )} ${t(`devicePage.${action}`, `${action}`)} "${deviceName}"`,
    ...
    const handleEditRecord = (record: any) => {
      ...
      router.push(`/ams/instrument/device/${record.id}`);
    };
    ...
    const fetchSampleStatisticsData = async (taskData: Task) => {
      ...
      if (
        ...
        if (taskData.type === "renderSampleStatistics") {
          sampleStatisticsUrl += `?asset_type_id=${deviceTypeId.value}&asset_id=${deviceId}`;
        }
