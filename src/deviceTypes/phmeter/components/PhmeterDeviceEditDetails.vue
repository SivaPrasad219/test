          ...
          >
            ...
            >
              ...
              <a-descriptions-item
                ...
                :span="2"
              >
                {{ lastCalibrationTime(device?.info?.calibration?.next) }}
              </a-descriptions-item>
              <a-descriptions-item
...
const fetchDeviceDetailsById = async () => {
  ...
  try {
    ...
    const isActiveDevice = response.data[0].is_active;
    if (!isActiveDevice) {
      navigateToDevices();
      return;
    }
      ...
      if (data.asset_id && subscribedDeviceId.value !== data.asset_id) {
        if (subscribedDeviceId.value) {
          socketService.leaveRooms([
            `instrument/phmeter/${subscribedDeviceId.value}/heartbeats/basic`,
          ]);
        }
        socketService.subscribeToRooms([
          `instrument/phmeter/${data.asset_id}/heartbeats/basic`,
        ]);
...
const navigateToDevices = () => {
  router.push("/phmeter/instrument");
};
