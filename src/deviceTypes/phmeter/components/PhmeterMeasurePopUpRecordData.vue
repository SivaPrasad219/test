...
<script lang="ts">
import { defineComponent, toRefs, ref, watch, onMounted, computed } from "vue";
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const updateData = () => {
      ...
      const measureTaskObj: any = measureTaskData.value || {};
      // let siteTimezone =
      //   measureTaskData.value?.meta?.asset_info?.site_info?.site_timezone ||
      //   "UTC";

      //console.log("::::::::::::;taskDataObj", taskDataObj);

      // Retrieve data
      const sampleCumulativeData = {
        pH: "5.85 pH",
        Temperature: "24.6 °C ATC",
        Signal: "63.7 mv",
        "Endpoint Duration": "Manual (24s)", // Fixed key formatting
      };
      console.log("----- cum -----", sampleCumulativeData);
      ...
      measureData.value = [
        ...
        {
          key: "nice_name",
          label: translateLabel("Equipment Method"),
          value:
            getValueByDataIndexKey(measureTaskObj, "sample_type_label") || "-",
        },
        {
          key: "id",
        ...
        {
          key: "volume",
          label: translateLabel("Measurement Type"),
          value: formatVolume({
        ...
        //   value:
        //     getValueByDataIndexKey(measureTaskObj, "meta.input.location") ||
        //     "-",
        // },
        {
          key: "period",
          label: translateLabel("Module"),
          value: formatTimeToSeconds(measureTaskObj.data?.data?.period) || "-",
        },
        {
          key: "period",
          label: translateLabel("Module Serial #"),
          value: formatTimeToSeconds(measureTaskObj.data?.data?.period) || "-",
        },
        {
          key: "period",
          label: translateLabel("Sensor"),
          value: formatTimeToSeconds(measureTaskObj.data?.data?.period) || "-",
        },
        {
          key: "period",
          label: translateLabel("Sensor Serial #"),
          value: formatTimeToSeconds(measureTaskObj.data?.data?.period) || "-",
        },
        {
          key: "operator",
      ...
      if (
        ...
        measureData.value.push({
          key: "location",
          label: translateLabel("Location"),
          value: measureTaskObj.meta.input.location
            ? measureTaskObj.meta.input.location
            : "-",
        });
      }
      ...
      if (
        ...
        measureData.value.push({
          key: "counts",
          label: translateLabel("Count Limits"),
          value: measureTaskObj.meta.input.count_limits,
        });
      }
      ...
      if (
        ...
        measureData.value.push({
          key: "flow_limits",
          label: translateLabel("Flow Limits"),
          value: measureTaskObj.meta.input.flow_limits,
        });
      }
        ...
        },
        {
          ...
          value:
            formatDate(
              getValueByDataIndexKey(
                measureTaskObj,
                "meta.asset_info.info.calibration.last"
              )
            ) || "-",
        },
      ];
      ...
      // Update measure info data
      const shouldLabelBeBold = (label: string): boolean => {
        return label === translateLabel("Measure Info");
      };

      const measureInfoDataTmp = (() => {
        // Process cumulativeData
        ...
        // console.log("::::::cumulativeDataMapped", cumulativeDataMapped);

        // Process differentialData
        const differentialDataMapped = Array.isArray(sampleDifferentialData)
          ? sampleDifferentialData.map((item: any, index: any) => ({
        ...
        const dataToDisplay = [
          ...(cumulativeDataMapped.length > 0
            ? [
                {
                  label: translateLabel("Measure Info"),
        ...
        // Prepare result array with bold labels where necessary
        const result: any[] = [];
        ...
        // Add boldLabel property based on label
      ...
      if (
        ...
        console.log(
          "dataPageHeader.valuedataPageHeader.value",
          dataPageHeader.value
        );
      }
      if (
        measureTaskObj?.meta?.input &&
