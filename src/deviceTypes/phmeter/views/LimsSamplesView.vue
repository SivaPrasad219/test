...
import ApcMeasurePopUpRecordData from "../components/PhmeterMeasurePopUpRecordData.vue";
...
const handleStartSample = (record: any) => {
  ...
  if (record?.id) {
    router.push(`/phmeter/lims/${record?.id}`);
  } else {
...
const handleSyncSample = (record: any) => {
  // console.log(" handleSelectedTableRowClick", record);
  if (record) {
    router.push(`/phmeter/lims-editor/${record?.id}`);
  } else {
    alert("Sample data not found. Please try another sample");
  }
};
